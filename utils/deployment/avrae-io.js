...
async function getAlias(aliasID) {
  const { data } = await get(`https://api.avrae.io/workshop/alias/${aliasID}/code`, {
      headers: header
  });
  return data.data;
}
...
async function getAliasIDs(workshop, group_name) {
  const result = await get(`https://api.avrae.io/workshop/collection/${workshop}/full`, {
      headers: header
  });
...
async function updateAlias(content, version, aliasID) {
  ...
  return put(`https://api.avrae.io/workshop/alias/${aliasID}/active-code`, {"version" : version}, {
    headers: header
  });
}

module.exports = { getGVar, updateGVar,
    getAlias, updateAlias, getAliasIDs
 }